...
#define UPDATE_MALLOC_TAGS(i, j){\
				i->sizeAndTags = i->sizeAndTags | (j);\
				*(size_t*)UNSCALED_POINTER_ADD(i, SIZE(i->sizeAndTags)-WORD_SIZE) = i->sizeAndTags;\
				}

#define UPDATE_FREE_TAGS(i, j){\
				i->sizeAndTags = i->sizeAndTags ^ (j);\
				*(size_t*)UNSCALED_POINTER_ADD(i, SIZE(i->sizeAndTags)-WORD_SIZE) = i->sizeAndTags;\
				}
...
void* mm_malloc(size_t size)
{
	...
	// if no fit is found, get more memory and place the block
	if(ptrFreeBlock == NULL || SIZE(ptrFreeBlock->sizeAndTags)-reqSize <= MIN_BLOCK_SIZE)
	{
		requestMoreSpace(reqSize*2);
		ptrFreeBlock = FREE_LIST_HEAD;	
	}

	// need to only remove an appropriately sized block from the list
	if(SIZE(ptrFreeBlock->sizeAndTags) > reqSize && SIZE(ptrFreeBlock->sizeAndTags)-reqSize >= MIN_BLOCK_SIZE)
	{
		blockSize = SIZE(ptrFreeBlock->sizeAndTags) - reqSize;
		split = UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
		
		ptrFreeBlock->sizeAndTags = reqSize | (ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED) | (ptrFreeBlock->sizeAndTags & TAG_USED);
		
		split->sizeAndTags = blockSize | 2 | 0;
		*(size_t*)UNSCALED_POINTER_ADD(split, SIZE(split->sizeAndTags) - WORD_SIZE) = split->sizeAndTags;
		
		split->prev = ptrFreeBlock->prev;
		split->next = ptrFreeBlock->next;
		if(ptrFreeBlock->next)
		{
			ptrFreeBlock->next->prev = split;
		}
		ptrFreeBlock->next = split;
		
		UPDATE_MALLOC_TAGS(ptrFreeBlock, 1);
		UPDATE_MALLOC_TAGS(split, 2);
	...
	else if(SIZE(ptrFreeBlock->sizeAndTags) == reqSize && SIZE(ptrFreeBlock->sizeAndTags)-reqSize>MIN_BLOCK_SIZE)
	{
		UPDATE_MALLOC_TAGS(ptrFreeBlock, 1);
		UPDATE_MALLOC_TAGS(ptrFreeBlock->next, 2);
		removeFreeBlock(ptrFreeBlock);
		return  UNSCALED_POINTER_ADD(ptrFreeBlock, WORD_SIZE);
	}
}
...
void mm_free(void *ptr)
{
	...
	blockInfo = ptr;
	blockInfo = UNSCALED_POINTER_SUB(blockInfo, WORD_SIZE);
	
	payloadSize = SIZE(blockInfo->sizeAndTags);
	followingBlock = UNSCALED_POINTER_ADD(blockInfo, payloadSize);

	if((blockInfo->sizeAndTags & 1) != 1)
	{
		return;
	}
	else
	{
		UPDATE_FREE_TAGS(blockInfo, 1);
		UPDATE_FREE_TAGS(followingBlock, 2);
	}
